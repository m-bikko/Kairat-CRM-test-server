...
export default function BlogPostPage({ params: { locale, slug } }: BlogPostPageProps) {
  ...
  // Форматирование контента с поддержкой простого HTML
  const formatContent = (content: string) => {
    // Базовая обработка переносов строк и параграфов
    return content
      .split('\n\n')
      .map((paragraph, index) => (
      ...
      <article className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="prose prose-lg max-w-none mb-8">
          <div className="text-gray-900">
            {formatContent(getLocalizedText(blog.content, currentLocale))}
          </div>
        </div>
